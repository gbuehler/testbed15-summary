...
The work demonstrated that prioritized delta updates of datasets can be served using a transactional extension of the OGC API – Features as well as an OGC API – Processes implementation in front of existing WFS instances.The ER concludes with a key recommendation that the OGC investigate a common approach for delta updates across existing and in development OGC APIs.
